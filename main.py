...
def wingAverage(tempArray):
    ...
    for j in range(0, 8):
        if int(tempArray[j]) > 0:
            evenTotal = evenTotal + 1

        elif int(tempArray[j]) < 0:
            oddTotal = oddTotal + 1
...
while True:
    ...
    deltaangle= 0
    #this loop is going to run until the robot is swutched off. The Robot can either move left right or continue straight for now


    #1stly, we're going to have to access the video and then find the angles from that.

    #------------------------------------------


    for i in range(0,8):
        #this is going to run in a continuous while loop getting the corresponging dlata angles.
        ##insert image processing code that outputs the deltaangle.:
        #------------------------------------------
        ...
        #------------------------------------------
        #code that will take the angle and output a 1 -1 or 0.
        tempArray[i] = deltaangle



    #we now have 4 angles so need to work out an average of these angles.
    #use function wingaverage

    turningNum = wingAverage(tempArray)
